...
// Triangle default values
const vec3 DEFAULT_COLOR = vec3(1.0f, 1.0f, 1.0f);
const float DEFAULT_SCALE = 1.0f;
const vec2 DEFAULT_POSITION = vec2(0.0f);
...
A0::A0()
	: m_shape_color(DEFAULT_COLOR),
	  m_shape_translation(DEFAULT_POSITION),
	  m_shape_size(DEFAULT_SCALE),
...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		// Resets the triangle's colour, scale, rotation, and position to their default values
		...
		// Rotation slider, 2 PI radians in a circle
